     ...
     <STYLE>
      ...
      <SCRIPT for="window" event="onload"><xsl:comment><![CDATA[
        ...
        <TABLE width="100%" cellspacing="0">
          ...
          <TR>
<!--          <TD>
			<input type="image" name="search" src="/linux/imgcso01.gif"></TD>
-->
            <TD class="fntPagina"/>
            <TD class="fntPagina">
          ...
          <TR>
            <TD class="fntLeftPag" width="120" valign="top">
              ...
              <P><xsl:eval>totalREP(this)</xsl:eval></P>
  ...
  <xsl:template match="MainTag">
    ...
    <TABLE STYLE="font:8pt Verdana; background-color:white">
      ...
      <TCabecalho>      
        ...
        <TD width="240"><DIV class="fntCabecTab" onClick="Classifica('MQ_QN_ORIGEM')">MQ_QN_ORIGEM</DIV></TD>
		<TD width="200"><DIV class="fntCabecTab" onClick="Classifica('MQ_DATETIME')">MQ_DATETIME</DIV></TD>
		<TD width="200"><DIV class="fntCabecTab" onClick="Classifica('NU_OPE')">NU_OPE</DIV></TD>
      ...
      <xsl:for-each select="EntryTag" order-by="DB_DATETIME">
        <TR>
          ...
          <TD><DIV class="fntLinhaTab"><xsl:value-of select="MSG"/></DIV></TD>    
        </TR>
      </xsl:for-each>
    </TABLE>
  </xsl:template>
  ...
  <xsl:script><![CDATA[
    ...
    {
      totalGeral = 0; 
      totalTrafegado = node.selectNodes("/MainTag/EntryTag/STATUS_MSG");      
    ...
    function totalREP(node)
    {
      ...
      return formatNumber(totalREP, "000") + " REPs";
    }
