...
void CIFReq::RunThread(LPVOID MainSrv)
{
...
bool CIFReq::RunInitDBeMQ()
{
    ...
    strcpy(m_szQueueName, pMainSrv->pInitSrv->m_MqQlIFCidadeReq);
    strcpy(m_od.ObjectName, pMainSrv->pInitSrv->m_MqQlIFCidadeReq);
   ...
   if (m_OpenCode == MQCC_FAILED)
   {
	...
	try
	{
		...
		m_pRS->m_index = 2;
		m_pRS->m_nParams = 2;
		m_pRS->m_ParamMQ_QN_DESTINO = pMainSrv->pInitSrv->m_MqQrCidadeBacenReq;
	...
	if (rt)
	{
		...
		try
		{
			...
			m_pDb1->ExecuteSQL(m_pRS->m_sIndex3);
			m_pDb1->ExecuteSQL(m_pRS->m_sTrigger);
			m_pRS->m_index = 2;
			m_pRS->m_nParams = 2;
			m_pRS->m_ParamMQ_QN_DESTINO = pMainSrv->pInitSrv->m_MqQrCidadeBacenReq;
			m_pRS->m_ParamFLAG_PROC = "N";
...
bool CIFReq::UpdateMQeDB(CIFAppRS *rs)
{
	bool erro = false;
	...
	switch (m_CompCode)
    {
     ...
     pMainSrv->pInitSrv->m_WriteLog(m_szTaskName,8061,TRUE,&pMainSrv->pInitSrv->m_QueueMgr,&pMainSrv->pInitSrv->m_MqQrCidadeBacenReq,&m_messlen,NULL,NULL);
	 DumpHeader(&m_md);
...
//------------------------------------------------------------------------------------------
	...
	m_pRS->m_MQ_QN_DESTINO = pMainSrv->pInitSrv->m_MqQrCidadeBacenReq;
...
//------------------------------------------------------------------------------------------
	...
	{  
        m_pRS->m_SECURITY_HEADER[i] = m_buffermsg[i];
    }

	return rc;
}

bool CIFReq::MontaDbRegLog()
{
	bool rc = false;
	int i = 0;
	LPBYTE pbyte;
...
//------------------------------------------------------------------------------------------
	...
	m_pRSLog->m_MQ_QN_ORIGEM = pMainSrv->pInitSrv->m_MqQrCidadeBacenReq;
...
//------------------------------------------------------------------------------------------
    ...
    for ( i=0; i< sizeof(SECHDR); i++) 
	{  
        m_pRSLog->m_SECURITY_HEADER[i] = m_buffermsg[i];
    }

	m_pRSLog->m_NU_OPE = m_pRS->m_NU_OPE;

	m_pRSLog->m_COD_MSG = m_pRS->m_COD_MSG;
...
bool CIFReq::MontaBufferMQ(CIFAppRS *rs)
{
	bool rc = false;
	BSTR bMsg = NULL;
...
//----------------------------------------------------------------
//  Montando mensagem do Header de Seguran�a
//----------------------------------------------------------------
	lpSecHeader = (LPSECHDR) &m_buffermsg[0];
	memset(lpSecHeader,0x00,sizeof(SECHDR));
	lpSecHeader->TamSecHeader[0] = 0x01;
	lpSecHeader->TamSecHeader[1] = 0x4C;
	if (pMainSrv->pInitSrv->m_SecurityEnable.Compare("S") == 0)
	{
		lpSecHeader->Versao = 0x01; // mensagem criptografada
		lpSecHeader->CodErro = 0x00; // sem erros
		lpSecHeader->AlgAssymKey = 0x01; // RSA com 1024 bits
		lpSecHeader->AlgSymKey = 0x01; // Triple-DES com 168 bits
		lpSecHeader->AlgAssymKeyLocal = 0x01; // RSA com 1024 bits
		lpSecHeader->AlgHash = 0x01; // MD5 
		lpSecHeader->CADest = 0x01; // Certsign
//		lpSecHeader->NumSerieCertDest ; // nr de s�rie certificado destino
		lpSecHeader->CALocal = 0x01; // Certsign
//		lpSecHeader->NumSerieCertLocal ; // nr de s�rie certificado local
		memset(lpSecHeader->SymKeyCifr,0xff,sizeof(lpSecHeader->SymKeyCifr)); // chave sim�trica cifrada
		memset(lpSecHeader->HashCifrSign,0xff,sizeof(lpSecHeader->HashCifrSign)); // hash cifrado - assinatura digital
	}

	m_messlen = sizeof(SECHDR);
	lenwrk = ((int) sizeof(SECHDR) + ((int) rs->m_MSG.GetLength() * (int) sizeof(WCHAR)) + 8 );
	if (lenwrk > pMainSrv->pInitSrv->m_MaxLenMsg)
	{
		pMainSrv->pInitSrv->m_WriteLog(m_szTaskName,8069,FALSE, &lenwrk,&pMainSrv->pInitSrv->m_MaxLenMsg,NULL,NULL,NULL);
		rc = true;
		return rc;
	}

	for (int i = rs->m_MSG.GetLength() - 1; i >= 0; i--)
	{
		TCHAR excludechar = rs->m_MSG.GetAt(i);
		switch (excludechar)
		{
			case 0x00:
			case 0x0D:
	...
	lenwrk = rs->m_MSG.GetLength();
	CopyMemory(&m_buffermsg[sizeof(SECHDR)],rs->m_MSG,lenwrk);

	if (AnsiToUnicode(lenwrk, &m_buffermsg[sizeof(SECHDR)]))
	{
	...
	if 	(lpSecHeader->Versao == 0x00)     // mensagem em claro
	{
		BSTR bstr = (BSTR) &m_buffermsg[sizeof(SECHDR)];
		if (rs->m_MSG.GetLength() > 0)
		{
			rc = ChecarXml(lenwrk, bstr);
			if (rc)
			{
			    pMainSrv->pInitSrv->m_WriteReg(m_szTaskName,FALSE,lenwrk,bstr);
				rc = true;
				return rc;
			}
			int lenwrk2 = lenwrk / 2;
			for (int i = 0;i < lenwrk2 ; i++)
			{
				bstr[i] = htons(bstr[i]);
			}
		}
		funcLog(lpSecHeader, lenwrk, &m_buffermsg[m_messlen]);
		m_messlen += lenwrk;
	}
	else
	{
		...
		else
		{
			...
			if (funcAssinar(lpSecHeader, lenwrk, &m_buffermsg[sizeof(SECHDR)]))
			{
				rc = true;
			}
			if (rc == 0)
			{
				if (funcLog(lpSecHeader, lenwrk, &m_buffermsg[sizeof(SECHDR)]))
				{
					rc = true;
				}
			}
			if (rc == 0)
			{
				if (funcCript(lpSecHeader, lenwrk, &m_buffermsg[sizeof(SECHDR)]))
				{
					rc = true;
				}
			}
...
//------------------------------------------------------------------------------------------
//  Task Desconverte de unicode para ascii nos testes    
//------------------------------------------------------------------------------------------
    ...
    if (!rc)
	{
		if (pMainSrv->pInitSrv->m_UnicodeEnable.Compare("N") == 0)
		{
			...
			{
				CopyMemory(&m_buffermsg[sizeof(SECHDR)],rs->m_MSG,lenwrk);
			    m_messlen = sizeof(SECHDR) + lenwrk; 
			}
		}
	}
...
bool CIFReq::ChecarXml(int lenmsg, BSTR &msg)
{
	bool rc = false;
	CString wrk;
	...
	if (lenmsg > 0 )
	{
		...
		if (!LoadDocumentSync(m_xmlDoc, (const char*) msg, lenmsg)) {
			rc = true;
			return rc;
		}
