...
/* Fixup for Windows support.  We need to include windows.h for various types
  ...
  // Fase 6: Comentado temporariamente para permitir compilação
  // Será resolvido permanentemente com migração para OpenSSL
...
typedef enum {						/* Algorithms */
	...
	CRYPT_ALGO_RC4,					/* RC4 */
	CRYPT_ALGO_RC5,					/* RC5 */
	CRYPT_ALGO_SAFER,				/* SAFER - will become AES once it's released */
...
typedef enum {						/* Keyset types */
	...
	CRYPT_KEYSET_SMARTCARD,			/* Smart card key carrier (deprecated) */
...
typedef enum {
	...
	CRYPT_OPTION_NET_SOCKS_USERNAME,	/* Socks server name */
	CRYPT_OPTION_NET_HTTP_PROXY,		/* Web proxy server */
	CRYPT_OPTION_NET_TIMEOUT,			/* Timeout for network accesses */
	...
	CRYPT_CERTINFO_OCSP_ARCHIVECUTOFF,		/* archiveCutoff */
	...
	CRYPT_DEVINFO_LOGGEDON,		/* Whether user is logged on */
	...
	CRYPT_CERTINFO_LAST_CERTINFO = CRYPT_CERTINFO_DN + 1,
...
#ifdef _CRYPT_DEFINED
	, CRYPT_CERTACTION_FIRST_USER = CRYPT_CERTACTION_REQUEST_CERT,
	CRYPT_CERTACTION_LAST_USER = CRYPT_CERTACTION_EXPIRE_CERT
#endif /* _CRYPT_DEFINED */
...
typedef enum {
	...
	CRYPT_KEYOPT_CREATE_CERTSTORE,	/* Create a CA cert store */
#ifdef _CRYPT_DEFINED
...
C_RET cryptCAAddRequest( C_IN CRYPT_KEYSET keyset,
						 C_IN CRYPT_CERTIFICATE certificate );
C_RET cryptCAGetRequest( C_IN CRYPT_KEYSET keyset,
						 C_OUT CRYPT_CERTIFICATE C_PTR certificate,
						 C_IN CRYPT_KEYID_TYPE keyIDtype,
						 C_IN void C_PTR keyID );
