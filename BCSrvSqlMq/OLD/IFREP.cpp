...
void CIFRep::RunThread(LPVOID MainSrv)
{
...
bool CIFRep::RunInitDBeMQ()
{
    ...
    strcpy(m_szQueueName, pMainSrv->pInitSrv->m_MqQlIFCidadeRep);
   ...
   if (m_OpenCode == MQCC_FAILED)
   {
	...
	try
	{
		...
		m_pRS->m_index = 2;
		m_pRS->m_nParams = 2;
		m_pRS->m_ParamMQ_QN_DESTINO = pMainSrv->pInitSrv->m_MqQrCidadeBacenRep;
...
bool CIFRep::UpdateMQeDB(CIFAppRS *rs)
{
	bool erro = false;
	...
	switch (m_CompCode)
    {
     ...
     pMainSrv->pInitSrv->m_WriteLog(m_szTaskName,8061,TRUE,&pMainSrv->pInitSrv->m_QueueMgr,&pMainSrv->pInitSrv->m_MqQrCidadeBacenRep,&m_messlen,NULL,NULL);
	 DumpHeader(&m_md);
...
//------------------------------------------------------------------------------------------
	...
	m_pRS->m_MQ_QN_DESTINO = pMainSrv->pInitSrv->m_MqQrCidadeBacenRep;
...
//------------------------------------------------------------------------------------------
	...
	m_pRSLog->m_MQ_QN_ORIGEM = pMainSrv->pInitSrv->m_MqQrCidadeBacenRep;
...
bool CIFRep::MontaBufferMQ(CIFAppRS *rs)
{
	LPBYTE pbyte1;
	LPBYTE pbyte2;
	m_messlen = 0;
	...
	if (rs->m_MQ_CORREL_ID.GetSize() > 0)
	{
		LPBYTE pbyte = (BYTE *) &m_md.CorrelId;
	    for ( i=0; i < sizeof(m_md.CorrelId) ; i++) 
		{
	        pbyte[i] = rs->m_MQ_CORREL_ID[i];
	    }
	}
	for (i=0; i< sizeof(SECHDR); i++) 
	{  
        m_buffermsg[i] = rs->m_SECURITY_HEADER[i];
    }

	m_messlen = sizeof(SECHDR);
	if (rs->m_MSG_LEN > 0)
	{
	    pbyte1 = m_buffermsg+sizeof(SECHDR); // - security header
        pbyte2 = (BYTE *) rs->m_MSG.GetBuffer(rs->m_MSG_LEN);
        memcpy(pbyte1,pbyte2,rs->m_MSG_LEN);
		m_messlen += rs->m_MSG_LEN;
		rs->m_MSG.ReleaseBuffer();
	}

//----------------------------------------------------------------
//  Montando mensagem 
//----------------------------------------------------------------
	if (m_messlen > pMainSrv->pInitSrv->m_MaxLenMsg)
	{
		pMainSrv->pInitSrv->m_WriteLog(m_szTaskName,8069,FALSE, &m_messlen,&pMainSrv->pInitSrv->m_MaxLenMsg,NULL,NULL,NULL);
		return true;
	}
