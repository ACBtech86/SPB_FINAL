...
void CIFRsp::RunThread(LPVOID MainSrv)
{
...
bool CIFRsp::RunInitDBeMQ()
{
    ...
    strcpy(m_szQueueName, pMainSrv->pInitSrv->m_MqQlIFCidadeRsp);
   ...
   if (m_OpenCode == MQCC_FAILED)
   {
	...
	try
	{
		...
		m_pRS->m_index = 2;
		m_pRS->m_nParams = 2;
		m_pRS->m_ParamMQ_QN_DESTINO = pMainSrv->pInitSrv->m_MqQrCidadeBacenRsp;
...
bool CIFRsp::UpdateMQeDB(CIFAppRS *rs)
{
	bool erro = false;
	...
	switch (m_CompCode)
    {
     ...
     pMainSrv->pInitSrv->m_WriteLog(m_szTaskName,8061,TRUE,&pMainSrv->pInitSrv->m_QueueMgr,&pMainSrv->pInitSrv->m_MqQrCidadeBacenRsp,&m_messlen,NULL,NULL);
	 DumpHeader(&m_md);
...
//------------------------------------------------------------------------------------------
	...
	m_pRS->m_MQ_QN_DESTINO = pMainSrv->pInitSrv->m_MqQrCidadeBacenRsp;
...
//------------------------------------------------------------------------------------------
	...
	m_pRSLog->m_MQ_QN_ORIGEM = pMainSrv->pInitSrv->m_MqQrCidadeBacenRsp;
...
bool CIFRsp::MontaBufferMQ(CIFAppRS *rs)
{
	bool rc = false;
	BSTR bMsg = NULL;
...
//		lpSecHeader->NumSerieCertLocal ; // nr de s�rie certificado local
	...
	lenwrk = ((int) sizeof(SECHDR) + ((int) rs->m_MSG.GetLength() * (int) sizeof(WCHAR)) );
...
bool CIFRsp::ChecarXml(int lenmsg, BSTR &msg)
{
	bool rc = false;
	CString wrk;
