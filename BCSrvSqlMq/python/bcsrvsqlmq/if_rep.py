# if_rep.py - IF Report task (port of IFRep.cpp/h)
#
# IF (local institution) task that polls the database for pending report
# records and sends them as MQ messages via MQPUT to the remote queue
# QRCidadeBacenRep.  This is the outbound path: DB -> MQ.
#
# Key difference from IFReq/IFRsp: MontaBufferMQ copies the existing
# security header and raw message from the DB record (no sign/encrypt),
# and the CorrelId from the DB record is set on the outgoing MQ message.
# MsgType is MQMT_REPORT.
...
class CIFRep(CThreadMQ):
    """IF Report task - reads pending DB records and sends to MQ (port of CIFRep)."""
    ...
    def run_init_db_mq(self) -> bool:
        ...
        # Queue names
        local_queue_name = init_srv.m_MqQlIFCidadeRep
        self.m_szQueueName = local_queue_name.ljust(48)
        remote_queue_name = init_srv.m_MqQrCidadeBacenRep
    ...
    def processa_queue(self):
        """Poll DB for pending report records (flag_proc='P'), build MQ
    ...
    # ------------------------------------------------------------------
    # Build MQ message buffer  (report-specific: raw copy from DB)
    # ------------------------------------------------------------------
    def monta_buffer_mq(self, rs: CIFAppRS) -> bool:
        """Copy security header and raw message from DB record into
        m_buffermsg.  Also set CorrelId from DB record on the MD.
        ...
        # Copy security header from DB record
        sec_hdr_data = rs.m_SECURITY_HEADER or b'\x00' * SECHDR_SIZE
        if len(sec_hdr_data) < SECHDR_SIZE:
            sec_hdr_data = sec_hdr_data.ljust(SECHDR_SIZE, b'\x00')
        self.m_buffermsg[:SECHDR_SIZE] = sec_hdr_data[:SECHDR_SIZE]
        self.m_messlen = SECHDR_SIZE

        # Copy raw message payload
        if rs.m_MSG_LEN and rs.m_MSG_LEN > 0:
            msg_bytes = rs.m_MSG.encode('latin-1', errors='replace') if isinstance(rs.m_MSG, str) else rs.m_MSG
            payload_len = min(len(msg_bytes), rs.m_MSG_LEN)
            self.m_buffermsg[SECHDR_SIZE:SECHDR_SIZE + payload_len] = msg_bytes[:payload_len]
            self.m_messlen += payload_len

        # Check total length
        if self.m_messlen > init_srv.m_MaxLenMsg:
            write_log(self.m_szTaskName, 8069, False,
                      f'Message too large: {self.m_messlen} > {init_srv.m_MaxLenMsg}')
            return True
    ...
    def update_mq_and_db(self, rs: CIFAppRS) -> bool:
        ...
        md['MsgType'] = pymqi.CMQC.MQMT_REPORT
        ...
        # Copy CorrelId from DB record (report messages carry the original CorrelId)
        if rs.m_MQ_CORREL_ID and len(rs.m_MQ_CORREL_ID) > 0:
            correl_id = rs.m_MQ_CORREL_ID
            if len(correl_id) < 24:
                correl_id = correl_id.ljust(24, b'\x00')
    ...
    def update_db_reg_app(self) -> bool:
        ...
        rs.m_STATUS_MSG = 'N'
        rs.m_FLAG_PROC = 'E'
        rs.m_MQ_QN_DESTINO = self.pMainSrv.pInitSrv.m_MqQrCidadeBacenRep
    ...
    def monta_db_reg_log(self) -> bool:
        ...
        remote_queue_name = self.pMainSrv.pInitSrv.m_MqQrCidadeBacenRep
    ...
    def _update_app_record_in_db(self):
        ...
        try:
            cursor.execute(sql, (
                ...
                rs.m_DB_DATETIME,
                rs.m_COD_MSG,
                self.pMainSrv.pInitSrv.m_MqQrCidadeBacenRep,
            ))
        finally:
            cursor.close()
